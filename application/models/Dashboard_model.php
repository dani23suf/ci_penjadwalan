...
ON tbl_anggotadatang.id_jadwal = tbl_jadwal.id_jadwal WHERE id_user = $id  GROUP BY YEARWEEK(tanggal)";
        ...
        public function getDashboardmonthly($id)
        {
                ...
                INNER JOIN tbl_jadwal
                ON tbl_anggotadatang.id_jadwal = tbl_jadwal.id_jadwal WHERE id_user = $id  GROUP BY MONTH(tanggal)";
                return  $this->db->query($bulan)->result_array();
        }
        public function getDashboardyear($id)
        {

                $tahun = "SELECT YEAR(tanggal)  AS bulan, COUNT(*) AS Jumlah_tahunan FROM tbl_anggotadatang 
                INNER JOIN tbl_jadwal
